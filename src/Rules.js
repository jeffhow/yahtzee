...
class FullHouse extends Rule {
  evalRoll = dice => {
    ...
    // return (freqs.length === 2) && (freqs.reduce( (a,c) => { return a+c }, 0) === 5) ? this.score : 0;
  }
}
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    const d = [...new Set( dice.sort( (a,b) => a-b ) )];
    let total = 0;
...
const threeOfKind = new SumDistro({ count: 3, description: "Sum of 3 matching dice" });
const fourOfKind = new SumDistro({ count: 4, description: "Sum of 4 matching dice" });
